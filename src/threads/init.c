...
int pintos_init (void) NO_RETURN;                       //function decleration
...
pintos_init (void)
{
  ...
  /* Clear BSS. */                    
  /*In computer programming, the "BSS" segment is a section of memory used
   for storing uninitialized data. The term "BSS" stands for "Block Started by Symbol," 
   and it's a common segment found in the memory layout of a program or an operating system kernel.   
  */
    ...
    while(true) {
     ...
     printf("\n");
      ...
      if(strcmp(line, "whoami") == 0) {
        ...
        printf("%lu seconds passed since Unix epoch.\n", rtc_get_time() );
      }

      else if(strcmp(line, "ram") == 0) {
        printf("%u of ram available for the OS\n", init_ram_pages * PGSIZE / 1024);
      }
        ...
        printf("Priority of the current thread is %u \n", thread_get_priority());
      }
